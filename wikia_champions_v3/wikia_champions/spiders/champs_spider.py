...
from wikia_champions.items import URLItems
import re
#from pprint import pprint

class URLTest4(BaseSpider):
    name="linkurl"
    ...
    def parse(self,response):
        ...
        urls = sel.xpath('//div[@class="nav-buttons"]/span/a[@rel="nofollow next"]/@href').extract()
        for url in urls:
            item = URLItems()
            item['url'] = url
            yield item
...
class ChampHomepageSpider(BaseSpider):
    ...
    def parse(self,response):
        ...
        #item['urldoc'] = sel.xpath('//text()').extract() #get all the text in a html page
